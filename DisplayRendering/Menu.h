...
PartyConfig run()
{
    ...
    while (m_window.isOpen() && m_screen != MenuScreen::DONE)
    {
        ...
        bool         clicked   = mouseDown && !m_prevClick;  // ← edge trigger
        m_prevClick            = mouseDown;
...
void loadCharacterAssets()
{
    ...
    CharLoad defs[4] = {
        { "../Players/Chrono_sprite_frame1.png", { 0, 0, 0, 0 } },
...
void tickLevelSelect(sf::Vector2i mouse, bool clicked)
{
    ...
    // ── Back button — positioned to NOT overlap title text ────────────────
    ...
    LevelEntry levels[] = {
        ...
        { 2, "HECKRAN CAVE",      "Depths of magic and madness.",        900.f, 550.f },
            
        { 3, "MANORIA CATHEDRAL", "Dark halls, darker intentions.",      450.f, 250.f }
    };

    constexpr float ENTRY_W = 220.f;
    constexpr float ENTRY_H = 64.f;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (over && clicked)
        {
            ...
            m_result.selectedLevel = m_selectedLevel;   //  sync immediately
    ...
    void tickPartySelect(sf::Vector2i mouse, bool clicked)
    {
        ...
        drawText("Selected: " + std::to_string(m_result.numPlayers()) + " / 4",
        ...
        {
            m_result.players.fill(PlayerType::NONE);
        ...
        bool hasPlayers = m_result.numPlayers() > 0;
                       ...
                       sf::Vector2i mouse, bool clicked)
{
    ...
    if (m_charAssets[i].loaded)
    {
        ...
        float scaledH      = bnds.height * scale;
